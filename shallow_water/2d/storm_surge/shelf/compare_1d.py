...
# encoding: utf-8
r"""
Compare 2d shelf runs with 1d shelf runs
...
for field in [0,1,2,3]:
    ...
    prefix = "ml_%sd_e%s_m%s_%s" % (1,method_1d,mx_1d,"shelf")
    out_dir = ''.join((prefix,"_output"))
    path_1d = os.path.join(base_path,'multi_layer_1d','shelf',out_dir)
    ...
    for method in [1,2,4]:
        prefix = "ml_%sd_e%s_m%s_%s" % (2,method,mx_2d,"shelf_redux")
        out_dir = ''.join((prefix,"_output"))
        path_2d = os.path.join(base_path,'multi_layer_2d','shelf_redux',out_dir)
...
fig.savefig(os.path.join(base_path,'multi_layer_2d','shelf_redux','comparison.pdf'))