...
C         dtdy = dt/dy
C         do 50 m=1,nvar
C         do 50 i=mbc+1,mitot-mbc
C         do 50 j=mbc+1,mjtot-mbc
C          if (mcapa.eq.0) then
C c
C c            # no capa array.  Standard flux differencing:
C 
C            q(i,j,m) = q(i,j,m)
C      &           - dtdx * (fm(i+1,j,m) - fp(i,j,m))
C      &           - dtdy * (gm(i,j+1,m) - gp(i,j,m))
C          else
C c            # with capa array.
C            q(i,j,m) = q(i,j,m)
C      &           - (dtdx * (fm(i+1,j,m) - fp(i,j,m))
C      &           + dtdy * (gm(i,j+1,m) - gp(i,j,m))) / aux(i,j,mcapa)
C          endif
...
C             do m=1,layers
C               index = 3*(m-1)
C               if (q(i,j,index+1) / rho(m) < drytolerance) then
C                   q(i,j,index+1) = max(q(i,j,index+1),0.d0)
C                   q(i,j,index+2) = 0.d0
C                   q(i,j,index+3) = 0.d0
C               endif
C             enddo
C         enddo
C       enddo
...
C c        # with source term:   use Godunov splitting
C          call src2(mx,my,nvar,mbc,mx,my,xlowmbc,ylowmbc,dx,dy,
C      &             q,maux,aux,time,dt)
C          endif
