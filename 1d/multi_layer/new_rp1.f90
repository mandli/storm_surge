...
!
    ...
    ! Locals
    integer :: i,j,m,mw,ipiv(4),info,layer_index,rare
    double precision :: eig_vec(4,4),A(4,4),delta(4),beta(4),lambda(4)
    double precision, dimension(4) :: flux_l,flux_r
    double precision, dimension(2) :: h_l,u_l,hu_l,h_r,u_r,hu_r,h_hat_l,h_hat_r
    double precision :: h_ave(2),b_l,b_r,tau,w_l,w_r
    ...
    do i=2-mbc,mx+mbc
        ...
        do j=1,2
            ...
            ! Steady state values used with the static linearized eigen solver
        ...
        ! Check to see if this Riemann problem has only one layer to solve for
        if (dry_state_r(2).and.dry_state_l(2)) then
            call sl_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,s,eig_vec)
            s(i,:) = lambda
        ...
        ! Check for inundation states - if these exist the solver will exit currently
        ...
        if (eigen_method == 1) then
            call linearized_eigen_system(h_hat_l,h_hat_r,u_l,u_r,b_r,b_l,rare,lambda,eig_vec)
        else if (eigen_method == 2) then
            call linearized_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,lambda,eig_vec)
        else if (eigen_method == 3) then 
            if (dry_state_l(2).and.dry_state_r(2)) then
                call sl_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,lambda,eig_vec)
            else
                call vel_diff_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,lambda,eig_vec)
            endif
        else if (eigen_method == 4) then
            if (dry_state_r(2).and.(.not.dry_state_l(2))) then
                call linearized_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,lambda,eig_vec)
            else
                call exact_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,lambda,eig_vec)
            endif
        else
        ...
        ! ====================================================================
        !  Calculate the flux jump - Dependent on dry states
        ! Dry state, bottom layer to right
        if (dry_state_r(2).and.(.not.dry_state_l(2))) then
            ! Inundation problem present
            if (rare == 1) then
        ...
        ! Left side dry, right side wet
        else if (dry_state_l(2).and.(.not.dry_state_r(2))) then
            ! Solve inundation problem
            if (rare == 2) then
        ...
        ! Both sides have a wet lower layer
        else
            momentum_transfer(1) =  g * rho(1) * h_ave(1) * (h_r(2) - h_l(2) + b_r - b_l)
            momentum_transfer(2) = -g * rho(1) * h_ave(1) * (h_r(2) - h_l(2)) + g * rho(2) * h_ave(2) * (b_r - b_l)
            flux_transfer_r = g * rho(1) * h_r(1) * h_r(2)
...
subroutine sl_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,s,eig_vec)

    use parameters_module, only: r,one_minus_r,g

    implicit none

    ! Input
    integer, intent(in) :: rare
    double precision, intent(in) :: h_l(2),h_r(2),u_l(2),u_r(2),b_r,b_l
...
end subroutine sl_eigen_system

subroutine linearized_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,s,eig_vec)
    ...
    ! Left state inundating right state
    if (rare == 1) then
        stop "This part of the eigen solver does not work!"
        ! Left going waves are same
        gamma_l = h_l(2) / h_l(1)
        gamma_r = h_r(2) / h_r(1)
        ...
        s(1) = -sqrt(g*h_l(1)*(1.d0+alpha(1)))
        s(2) = -sqrt(g*h_l(1)*(1.d0+alpha(2)))
        
        ! Right states
        s(3) = u_l(2) + 2.d0 * sqrt(g * h_l(2))
        s(4) = u_r(1) + sqrt(g*h_r(1)) ! <-- This is the speed we want anyway
    ...
    ! Right state inundating left state
    else if (rare == 2) then
        stop "Right state inundating left not handled!"
    ! Otherwise
    else
        gamma_l = h_l(2) / h_l(1)
        gamma_r = h_r(2) / h_r(1)
        ...
        s(1) = -sqrt(g*h_l(1)*(1.d0+alpha(1)))
        s(2) = -sqrt(g*h_l(1)*(1.d0+alpha(2)))
        s(3) = sqrt(g*h_r(1)*(1.d0+alpha(3)))
        s(4) = sqrt(g*h_r(1)*(1.d0+alpha(4)))
...
end subroutine linearized_eigen_system


subroutine vel_diff_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,s,eig_vec)
...
end subroutine vel_diff_eigen_system

subroutine exact_eigen_system(h_l,h_r,u_l,u_r,b_r,b_l,rare,s,eig_vec)
    ...
    ! Local
    integer :: j,info
    double precision :: h_ave(2),u_ave(2),A(4,4)
...
end subroutine exact_eigen_system
